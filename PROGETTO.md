...
Esito Fase A: completata. Fase 2: vertical slice WordPress 0.2.8 installata, attiva e collaudata. Sono stati verificati homepage, pagina Santiago, pannello eventi, salvataggio delle bozze, caricamento condizionale degli asset, preset dei dati, migrazione compatibile della tabella partecipanti e collegamento HMAC Workspace. Le nuove iscrizioni vengono salvate prima nel database WordPress e poi replicate su Workspace con chiave di idempotenza; gli errori remoti restano in stato `PENDING` e vengono ritentati in piccoli lotti. Homepage e pagina Santiago non caricano asset del modulo. I dati estesi usano allowlist e validazione server, sono consultabili nel dettaglio iscrizione con etichette leggibili e le date di nascita future o anteriori a 120 anni vengono rifiutate; la bozza 2027 mantiene il profilo Minimo. Le prime bozze di attività ed evento esistono nel pannello, ma non è stato pubblicato alcun modulo e nessun invio reale è attivo.
...
- dettaglio dei partecipanti nel pannello e risposte configurabili conservate in `mi_participants.extra_json`: visualizzazione con etichette leggibili completata nella 0.2.8; filtri ed esportazione restano da aggiungere coerentemente con le capability e l'ambito attività;
