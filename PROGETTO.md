...
Esito Fase A: completata. Fase 2: WordPress 0.5.1 e Apps Script 1.1.0 installati e collegati; la 0.5.2 è in preparazione con gratuità esplicita, escaping contestuale delle email, normalizzazione economica unica e replica Workspace asincrona dopo il salvataggio locale. Il controllo firmato non invasivo dello schema Workspace è stato collaudato con esito positivo. Il deployment resta in modalità `ANTEPRIMA`. Non contiene coordinate operative e non riscuote pagamenti. La coda email resta `PREVIEW`. Pagamenti e invii reali non sono operativi.
