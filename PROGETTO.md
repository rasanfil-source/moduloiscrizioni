...
Esito Fase A: completata. Fase 2: WordPress 0.5.2 e Apps Script 1.1.0 installati e collegati; la 0.5.3 è in preparazione con stato operativo Workspace e riaccodamento protetto per singola replica. Gratuità esplicita, validazione delle combinazioni economiche e collegamento firmato allo schema Workspace `1.1.0` sono già stati collaudati senza creare iscrizioni. I segnaposto email usano escaping contestuale e la replica Workspace viene accodata dopo il salvataggio locale. Il deployment resta in modalità `ANTEPRIMA`. Non contiene coordinate operative e non riscuote pagamenti. La coda email resta `PREVIEW`. Pagamenti e invii reali non sono operativi.
