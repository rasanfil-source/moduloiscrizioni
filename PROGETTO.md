...
Stato: implementazione revisionata — WordPress 3.21.0 / Workspace 1.8.0
Data: 27 agosto 2026
...
- I modelli storici dei fogli restano dati di riferimento: non si cancellano né si rinominano automaticamente. La Segreteria usa viste operative, non report come strumento principale: profilo minimo, quota unica, servizi multipli e viaggio complesso. Il profilo iniziale deriva dai dati richiesti e dalle opzioni dell'evento; una configurazione specifica dell'evento può sostituirlo.
- Le viste operative mostrano sempre poche colonne essenziali e comprimono i dettagli correlati. Per esempio `Incassato` può espandere `Contanti`, `Bonifico` e `Carta/PayPal`; documenti, servizi e sistemazioni seguono lo stesso principio. La modifica conserva una conferma esplicita e uno storico append-only.
- La prima vista generata per un evento è dimostrativa. Dopo l'approvazione dell'operatore, struttura e righe vengono materializzate in una scheda dedicata e le aperture successive leggono direttamente quella copia rapida. `Aggiorna dati` conserva le colonne approvate; `Rigenera struttura` ricalcola anche modello e colonne. Entrambe le azioni richiedono conferma esplicita.
- Il portale WordPress autonomo è l’unica `Segreteria eventi` e riunisce eventi, creazione, schede delle iscrizioni e comunicazioni. Il segretario opera su tutti i gruppi; ogni operatore accede con credenziali personali WordPress soltanto agli eventi dei gruppi assegnati e non al normale pannello amministrativo. L'operatività collettiva confluisce in un file Google Sheets dedicato per ogni evento, registrato in `Fogli iniziative`, con colonne scelte dal profilo dell'evento e dettagli raggruppabili. `DB_MODULI` resta l'archivio centrale: gli aggiornamenti verso il foglio evento e il recepimento delle modifiche richiedono azioni distinte, conferma esplicita e registrazione nel `Registro controlli`.
- Il gestore è facoltativo: la sua assenza non impedisce la pubblicazione o la creazione del foglio. Senza un responsabile univoco con email valida, la comunicazione viene preparata per l’email segreteria eventi configurata privatamente in Collegamento Workspace, senza creare utenti fittizi né modificare il mittente del sito. Se tale indirizzo manca, l’evento viene comunque pubblicato con un avviso. Non si aggiungono condivisioni e gli accessi Google esistenti restano invariati: i ruoli WordPress non concedono automaticamente permessi Google Drive. Tutte le comunicazioni rispettano la modalità email generale e in ANTEPRIMA restano anteprime.
- I fogli operativi prodotti dal plugin sono raccolti nella cartella Drive `EVENTI`, collocata accanto a `DB_MODULI`. Al suo interno `EVENTI PASSATI` contiene esattamente i fogli degli eventi mostrati nella sezione omonima del portale. Un riallineamento periodico idempotente sposta anche i documenti preesistenti senza ricrearli, cambiarne il collegamento o perdere dati.
