...
Esito Fase A: completata. Fase 2: vertical slice WordPress 0.3.1 pronta per il collaudo sul sito. La versione estende l'editor 0.3.0 conservando in ogni nuova voce locale dell'outbox una copia già risolta del modello e l'impronta della revisione; il pannello può mostrare HTML, testo semplice e metadati storici senza ricalcolarli dopo modifiche successive all'evento. Non esiste alcuna funzione di spedizione e la coda resta `PREVIEW`. Le funzionalità già collaudate comprendono salvataggio locale autorevole, replica Workspace idempotente, capienza, profili dati, dettaglio partecipanti, ricerca, filtro evento ed esportazione CSV limitata alle attività accessibili. L'evento 2027 risulta pubblicato nel tipo di contenuto amministrativo non pubblico; la pagina pubblica di riferimento non contiene lo shortcode né gli asset del modulo. Pagamenti e invii reali non sono operativi.
...
- editor e anteprima email: editor per evento completato nella 0.3.0 con segnaposto italiani, HTML limitato, testo semplice e dati sintetici; istantanea risolta e impronta della revisione nell'outbox completate nella 0.3.1; branding ereditato e prova controllata restano incrementi successivi;
