...
final class MI_Portal_Payments {
	...
	public static function search( $query ) {
		...
		if ( 'ALL' !== $scope && ! $scope ) return array();
		$where = 'ALL' === $scope ? '1=1' : 'event_id IN (' . implode( ',', array_map( 'absint', $scope ) ) . ')';
		$like = '%' . $wpdb->esc_like( $query ) . '%';
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT id,event_id,order_code,buyer_first_name,buyer_last_name FROM {$wpdb->prefix}mi_registrations WHERE {$where} AND (CONCAT(buyer_first_name,' ',buyer_last_name) LIKE %s OR CONCAT(buyer_last_name,' ',buyer_first_name) LIKE %s OR order_code LIKE %s) ORDER BY buyer_last_name,buyer_first_name,id LIMIT 30", $like, $like, $like ), ARRAY_A );
		return array_map( static function ( $row ) {
			return array( 'id' => (int) $row['id'], 'nome' => trim( $row['buyer_first_name'] . ' ' . $row['buyer_last_name'] ), 'codice' => $row['order_code'], 'evento' => get_the_title( (int) $row['event_id'] ) );
		}, $rows ?: array() );
	}
	public static function ajax() {
		...
		if ( 'search' === $operation ) wp_send_json_success( array( 'prenotazioni' => self::search( wp_unslash( $_POST['query'] ?? '' ) ) ) );
	...
	public static function render() {
		...
		?>
		<section class="mi-payments" data-mi-payments data-initial-order="<?php echo esc_attr( sanitize_text_field( wp_unslash( $_GET['mi_order'] ?? '' ) ) ); ?>" data-endpoint="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'mi_portal_payment' ) ); ?>">
		...
		<fieldset data-search-fields><label for="mi-payment-search">Nome, cognome o codice prenotazione</label>
