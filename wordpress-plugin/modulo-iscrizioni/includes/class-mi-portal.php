...
final class MI_Portal {
	...
	private static function create_view() {
		...
		<section class="mi-wizard-step"><span class="mi-portal-eyebrow">4 di 8</span><h2>Date e posti</h2><div class="mi-wizard-grid"><label>Apertura iscrizioni<input type="datetime-local" name="opens_at"></label><label>Data e ora di inizio<input type="datetime-local" name="starts_at" min="<?php echo esc_attr( $today_start ); ?>" required></label><label>Chiusura iscrizioni<input type="datetime-local" name="closes_at" required></label><label>Posti disponibili<input type="number" min="1" max="10000" name="capacity" value="30" required></label></div><p class="mi-portal-muted">La data di inizio non può essere precedente a oggi.</p><label class="mi-check"><input type="checkbox" name="waitlist_enabled" value="1"> Attiva automaticamente la lista d’attesa a esaurimento posti</label></section>
