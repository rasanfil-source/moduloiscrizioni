...
final class MI_Portal {
	...
	public static function portal_icon_links() {
		...
		$asset_url = MI_PLUGIN_URL . 'assets/portal-icon-';
		echo '<link rel="icon" type="image/png" sizes="32x32" href="' . esc_url( $asset_url . '32.png?ver=' . MI_VERSION ) . '">';
		echo '<link rel="icon" type="image/png" sizes="192x192" href="' . esc_url( $asset_url . '192.png?ver=' . MI_VERSION ) . '">';
		echo '<link rel="icon" type="image/png" sizes="512x512" href="' . esc_url( $asset_url . '512.png?ver=' . MI_VERSION ) . '">';
		echo '<link rel="apple-touch-icon" sizes="180x180" href="' . esc_url( $asset_url . '180.png?ver=' . MI_VERSION ) . '">';
	}
	...
	private static function handle_communication_action() {
		...
		$result = MI_Spedizione_Email::accoda_comunicazione_operativa( array(
			...
			'allow_operational' => 'PROVA' === MI_Spedizione_Email::modalita(), 'recipients' => $recipients,
		) );
		...
		$delivery_notice = $test_queued ? ' Invio previsto esclusivamente all’indirizzo configurato per i test, non ai destinatari reali.' : ' Comunicazioni preparate senza invio.';
	...
	private static function communications_view() {
		...
		echo '</select></label><label>Messaggio<textarea name="message" rows="8" maxlength="4000" placeholder="Punto di ritrovo, orario, cosa portare e altre indicazioni…"></textarea></label><button class="mi-primary" type="submit">' . ( $test_ready ? 'Prepara invio di test' : 'Prepara comunicazione' ) . '</button></form>';
