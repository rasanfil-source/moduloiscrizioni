...
final class MI_Portal {
	...
	public static function handle_actions() {
		...
		if ( isset( $_POST['event_email_style_present'] ) ) {
			$email_style = array();
			if ( ! empty( $_POST['event_email_identity_enabled'] ) ) $email_style = array( 'identity_name' => $_POST['event_email_identity_name'] ?? '', 'identity_detail' => $_POST['event_email_identity_detail'] ?? '', 'contact_email' => $_POST['event_email_contact'] ?? '', 'signature' => $_POST['event_email_signature'] ?? '' );
	...
	public static function render_virtual_page() {
		if ( ! empty( $_GET['mi_status'] ) && ! empty( $_GET['ordine'] ) && ! empty( $_GET['token'] ) ) {
			...
			if ( is_array( $verified ) && ! empty( $verified['collectible'] ) ) {
				global $wpdb;
				$registration = $wpdb->get_row( $wpdb->prepare( "SELECT id,event_id FROM {$wpdb->prefix}mi_registrations WHERE order_code=%s LIMIT 1", $code ), ARRAY_A );
				$participant = $registration ? $wpdb->get_row( $wpdb->prepare( "SELECT id,first_name,last_name FROM {$wpdb->prefix}mi_participants WHERE registration_id=%d AND status='ACTIVE' ORDER BY id LIMIT 1", $registration['id'] ), ARRAY_A ) : null;
				$prefill = $participant ? array( 'row' => (int) $participant['id'], 'nome' => $participant['first_name'], 'cognome' => $participant['last_name'] ) : array();
	...
	private static function handle_group_action( $action ) {
		...
		$email_style = array();
		if ( ! empty( $_POST['group_email_identity_enabled'] ) ) $email_style = array( 'identity_name' => $_POST['group_email_identity_name'] ?? '', 'identity_detail' => $_POST['group_email_identity_detail'] ?? '', 'contact_email' => $_POST['group_email_contact'] ?? '', 'signature' => $_POST['group_email_signature'] ?? '' );
	...
	private static function event_email_appearance_fields( $event_id, $group_id ) {
		...
		$identity = ! empty( array_intersect_key( $style, array_flip( array( 'identity_name', 'identity_detail', 'contact_email', 'signature' ) ) ) );
