...
final class MI_Portal {
	...
	private static function create_view() {
		...
		<section class="mi-wizard-step"><span class="mi-portal-eyebrow">5 di 8</span><h2>Quali dati chiediamo?</h2><p>Nome e cognome sono sempre obbligatori per ogni partecipante.</p><fieldset><legend>Dati aggiuntivi da compilare</legend><label class="mi-check"><input type="radio" name="participant_extra_scope" value="ONE" checked> Solo per uno degli iscritti (il primo, modificabile)</label><label class="mi-check"><input type="radio" name="participant_extra_scope" value="ALL"> Per tutti gli iscritti</label></fieldset><div class="mi-field-choice-list"><?php foreach ( $catalog as $key => $field ) : ?><div class="mi-field-choice"><label class="mi-check"><input type="checkbox" name="participant_fields[]" value="<?php echo esc_attr( $key ); ?>" data-mi-field="<?php echo esc_attr( $key ); ?>"> <?php echo esc_html( $field['label'] ); ?></label><label class="mi-check mi-field-required"><input type="checkbox" name="participant_required[]" value="<?php echo esc_attr( $key ); ?>" data-mi-required="<?php echo esc_attr( $key ); ?>"> Obbligatorio</label><small><?php echo esc_html( $field['help'] ?? '' ); ?></small></div><?php endforeach; ?></div></section>
