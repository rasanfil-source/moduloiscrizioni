...
final class MI_Portal {
	...
	public static function handle_actions() {
		...
		foreach ( array_slice( $bus_route_codes, 0, 12, true ) as $index => $route_label ) {
			...
			$route_slug = sanitize_title( $route_label );
			if ( ! $route_slug || isset( $known_bus_routes[ $route_slug ] ) ) continue;
			$route_price = self::parse_euro_cents( $bus_route_prices[ $index ] ?? '' );
			if ( is_wp_error( $route_price ) || $route_price < 0 ) continue;
			$known_bus_routes[ $route_slug ] = true;
			$service_options[] = array( 'code' => 'pullman-' . $route_slug, 'name' => 'Pullman' === $route_label ? 'Pullman' : 'Pullman — ' . $route_label, 'scope' => 'TICKET', 'price_cents' => $route_price, 'max_quantity' => 1 );
		}
	...
	private static function event_management_card( $event_id ) {
		...
		echo '<p class="mi-event-management__full-edit"><span>Vuoi modificare l’immagine, le domande, i servizi o altri particolari?</span><a class="mi-secondary" href="' . esc_url( add_query_arg( array( 'mi_portal_view' => 'create', 'mi_portal_edit' => $event_id ), self::base_url() ) ) . '">Tutti i dettagli</a></p></details>';
		if ( $active_count > 0 ) {
