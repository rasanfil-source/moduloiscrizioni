			...
			<form class="mi-registration__form" novalidate>
				<div class="mi-registration__step">
					...
					<?php foreach ( $event['ticket_types'] as $ticket ) : ?>
						<label class="mi-registration__ticket"><span><strong><?php echo esc_html( $ticket['name'] ); ?></strong><?php if ( 'CALCULATED' === $event['pricing_mode'] ) : ?><small><?php echo esc_html( number_format_i18n( $ticket['price_cents'] / 100, 2 ) ); ?> €</small><?php elseif ( 'ZERO' === $event['pricing_mode'] ) : ?><small>Gratuito</small><?php endif; ?></span><input type="number" min="0" max="<?php echo esc_attr( $ticket['max_per_order'] ); ?>" value="0" data-mi-ticket="<?php echo esc_attr( $ticket['code'] ); ?>" aria-label="Quantità <?php echo esc_attr( $ticket['name'] ); ?>"></label>
					<?php endforeach; ?>
					</div>
