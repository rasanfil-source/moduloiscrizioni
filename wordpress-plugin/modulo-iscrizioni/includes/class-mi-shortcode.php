		...
		<section id="<?php echo esc_attr( $instance_id ); ?>" class="mi-registration" style="--mi-primary:<?php echo esc_attr( $event['accent_color'] ); ?>;--mi-primary-dark:<?php echo esc_attr( self::darken_color( $event['accent_color'] ) ); ?>" data-mi-config="<?php echo esc_attr( wp_json_encode( $config ) ); ?>">
			...
			<form class="mi-registration__form" novalidate>
				<section class="mi-registration__step" data-mi-step="1" aria-labelledby="<?php echo esc_attr( $instance_id ); ?>-step-1"><h2 id="<?php echo esc_attr( $instance_id ); ?>-step-1" tabindex="-1">Scegli le iscrizioni</h2><p class="mi-registration__intro">Seleziona la quantità per ogni tipologia.</p>
					<div class="mi-registration__tickets"><?php foreach ( $event['ticket_types'] as $ticket ) : ?><?php $type_availability = $event['availability']['ticket_types'][ $ticket['code'] ] ?? array(); $type_remaining = $type_availability['remaining'] ?? null; $maximum = ( $event['waitlist_enabled'] || null === $type_remaining ) ? $ticket['max_per_order'] : min( $ticket['max_per_order'], $type_remaining ); ?><label class="mi-registration__ticket"><span><strong><?php echo esc_html( $ticket['name'] ); ?></strong><?php if ( 'FIXED' === $event['pricing_mode'] ) : ?><small>Quota di partecipazione: <?php echo esc_html( number_format_i18n( $event['fixed_price_cents'] / 100, 2 ) ); ?> €</small><?php elseif ( 'CALCULATED' === $event['pricing_mode'] ) : ?><small><?php echo esc_html( number_format_i18n( $ticket['price_cents'] / 100, 2 ) ); ?> €</small><?php elseif ( 'ZERO' === $event['pricing_mode'] ) : ?><small>Gratuito</small><?php endif; ?><?php if ( null !== $type_remaining ) : ?><small><?php echo esc_html( $type_remaining . ' disponibili per questa tipologia' ); ?></small><?php endif; ?></span><span class="mi-registration__quantity"><span>Quantità</span><input type="number" min="0" max="<?php echo esc_attr( max( 0, $maximum ) ); ?>" value="0" data-mi-ticket="<?php echo esc_attr( $ticket['code'] ); ?>" aria-label="Quantità <?php echo esc_attr( $ticket['name'] ); ?>" <?php disabled( 0 === $maximum && ! $event['waitlist_enabled'] ); ?>></span></label><?php endforeach; ?></div>
