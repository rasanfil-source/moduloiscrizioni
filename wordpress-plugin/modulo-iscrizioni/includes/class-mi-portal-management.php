...
final class MI_Portal_Management {
	...
	public static function render( $registration_id = 0 ) {
		...
		<label<?php echo $compact ? ' hidden' : ''; ?>><select data-period-select aria-label="Eventi attivi o passati"><option value="current" <?php selected( $period, 'current' ); ?>>Eventi attivi</option><option value="past" <?php selected( $period, 'past' ); ?>>Eventi passati</option></select></label>
		<label<?php echo $compact ? ' hidden' : ''; ?>>Evento<select data-event-select aria-label="Evento"><option value="">Scegli un evento</option><?php foreach ( $events as $event ) : ?><option data-period="<?php echo esc_attr( $periods[$event->ID] ); ?>" value="<?php echo esc_attr( $event->ID ); ?>" <?php selected( $event_id, $event->ID ); ?>><?php echo esc_html( $event->post_title ); ?></option><?php endforeach; ?></select></label>
