# Modulo Iscrizioni — versione 1.1.0
...
Il pacchetto descritto in questa pagina è la versione 1.1.0. Parte in modalità email `ANTEPRIMA`; la prova usa soltanto dati sintetici e la modalità `OPERATIVO` richiede prima una prova accettata dal sistema di posta. Le email fallite possono essere riaccodate manualmente dal pannello e quelle rimaste in elaborazione vengono recuperate automaticamente. Mostra inoltre i posti residui, segnala chiaramente il passaggio alla lista d’attesa, blocca il modulo quando i posti sono esauriti senza lista e ricontrolla la scadenza dentro la transazione degli ultimi posti.
