...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
	...
	const updateBookingLimit = () => {
	  ...
	  const scope = form.querySelector('[data-mi-participant-scope]');
	  if (scope) { scope.hidden = !multiple; scope.disabled = !multiple; }
	...
	copyEvent?.addEventListener('change', () => {
	  ...
	  if (copyEvent.value) url.searchParams.set('mi_copy_from', copyEvent.value); else url.searchParams.delete('mi_copy_from');
	  try { sessionStorage.setItem('mi-copy-title', JSON.stringify({path: url.pathname, title: form.elements.title.value})); } catch (_) {}
	  window.location.assign(url.href);
	});
	if (copyEvent) { try { const saved = JSON.parse(sessionStorage.getItem('mi-copy-title') || 'null'); sessionStorage.removeItem('mi-copy-title'); if (saved?.path === location.pathname) form.elements.title.value = saved.title; } catch (_) {} }
    ...
    if (rooms && !rooms.querySelector('[data-mi-accommodation-management]')) {
      ...
      const mode = form.dataset.miAccommodationManagementMode === 'PREBOOKED' ? 'PREBOOKED' : 'ON_DEMAND';
      management.querySelector('input[value="'+mode+'"]').checked = true;
    }
