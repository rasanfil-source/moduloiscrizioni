(() => {
  ...
  function reportWorkbook(rows) {
    if(!rows.length||rows.length>1048576||rows.reduce((n,r)=>Math.max(n,r.length),0)>16384)throw new Error('Il rapporto supera i limiti Excel.');
    ...
    const width=rows[0].length,end=column(width-1)+rows.length;
    const sheet='<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:'+end+'"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>'+rows[0].map((_,i)=>'<col min="'+(i+1)+'" max="'+(i+1)+'" width="24" customWidth="1"/>').join('')+'</cols><sheetData>'+rows.map((row,r)=>'<row r="'+(r+1)+'">'+row.map((v,c)=>{const ref=column(c)+(r+1),style=r===0?1:0;if(typeof v==='number'&&Number.isFinite(v))return '<c r="'+ref+'" s="'+style+'"><v>'+v+'</v></c>';if(String(v??'').length>32767)throw new Error('Un dato supera i 32767 caratteri consentiti da Excel.');return '<c r="'+ref+'" s="'+style+'" t="inlineStr"><is><t xml:space="preserve">'+xml(v)+'</t></is></c>';}).join('')+'</row>').join('')+'</sheetData><autoFilter ref="A1:'+end+'"/></worksheet>';
    const files={
      ...
      'xl/workbook.xml':'<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Rapporto partecipanti" sheetId="1" r:id="rId1"/></sheets></workbook>',
      'xl/_rels/workbook.xml.rels':'<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>',
      'xl/styles.xml':'<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><b/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFE8F1FC"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>',
    ...
    for(const [name,content] of Object.entries(files)){
      const filename=encoder.encode(name),data=encoder.encode('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'+content);let crc=0xffffffff;
      for(const byte of data){crc^=byte;for(let bit=0;bit<8;bit++)crc=(crc>>>1)^((crc&1)?0xedb88320:0);}crc=(crc^0xffffffff)>>>0;
      const local=header(30,0x04034b50),l=new DataView(local.buffer);l.setUint16(4,20,true);l.setUint16(12,33,true);l.setUint32(14,crc,true);l.setUint32(18,data.length,true);l.setUint32(22,data.length,true);l.setUint16(26,filename.length,true);
      const central=header(46,0x02014b50),d=new DataView(central.buffer);d.setUint16(4,20,true);d.setUint16(6,20,true);d.setUint16(14,33,true);d.setUint32(16,crc,true);d.setUint32(20,data.length,true);d.setUint32(24,data.length,true);d.setUint16(28,filename.length,true);d.setUint32(42,offset,true);
      parts.push(local,filename,data);directory.push(central,filename);offset+=local.length+filename.length+data.length;
    }
    const endRecord=header(22,0x06054b50),e=new DataView(endRecord.buffer);e.setUint16(8,6,true);e.setUint16(10,6,true);e.setUint32(12,directory.reduce((n,b)=>n+b.length,0),true);e.setUint32(16,offset,true);
  ...
  function init(root) {
    ...
    async function allPeople(){
      ...
      const load=async(reset=true)=>{const seq=++sequence;if(reset){offset=0;host.replaceChildren();}more.hidden=true;say('Caricamento iscrizioni…');
        try{const data=await request('all_people',{query:allQuery,period,offset,include_closed:allClosed?'1':'0'});if(ticket!==generation||seq!==sequence)return;
          ...
          for(const p of data.items){const state=p.status==='CANCELLED'?'CANCELLED':p.booking_status;host.insertAdjacentHTML('beforeend','<article class="mi-all-person"><div><strong>'+esc([p.last_name,p.first_name].join(' '))+'</strong><p>'+esc(p.event_title)+'</p><small>Iscrizione del '+esc(String(p.created_at).slice(0,10).split('-').reverse().join('/'))+(labels[state]?' · '+esc(labels[state]):'')+'</small></div><button type="button" data-open="'+esc(p.order_code)+'" data-open-event="'+Number(p.event_id)+'" data-person-focus="'+Number(p.number)+'">Gestisci</button></article>');}
      ...
      try{const data=await request('summary');if(ticket!==generation)return;
        ...
        if(features.rooms&&Object.keys(data.room_types||{}).length){
          ...
          const renderAssignments=()=>{
            ...
            }).join('')+'</tbody></table></div><p>Lascia il numero vuoto per togliere l’assegnazione. La sistemazione richiesta è il criterio dell’assegnazione.</p><div class="mi-room-assignment-actions mi-room-sticky-actions"><span data-room-assignment-status>Nessuna modifica</span><button type="button" data-discard-room-assignments>Annulla</button><button type="submit" class="mi-primary">Salva assegnazioni</button></div></form>';
            ...
            const updateAssignmentStatus=()=>{const changed=[...form.querySelectorAll('[data-room-person]')].filter(el=>el.value!==el.dataset.initial).length;form.querySelector('[data-room-assignment-status]').textContent=changed?(changed+' '+(changed===1?'assegnazione modificata':'assegnazioni modificate')):'Nessuna modifica';};
            form.onchange=e=>{if(!e.target.matches('[data-room-person]'))return;const input=e.target,type=types[input.dataset.roomType];validateNumber(input);input.closest('tr').querySelector('[data-room-preview]').textContent=input.value?type.prefix+input.value:'Da assegnare';updateAssignmentStatus();};
          ...
          changePanel.innerHTML='<summary>Cambia tipo di abitazione</summary><p>Seleziona le persone a cui cambiare tipo di alloggio e assegna contestualmente la stanza di destinazione.</p><form data-accommodation-form novalidate><label class="mi-accommodation-search">Cerca persona o sistemazione<input type="search" data-accommodation-search placeholder="Nome, cognome o sistemazione"></label><fieldset><legend>Elenco persone</legend>'+admitted.filter(p=>(p.options||[]).some(o=>String(o.code||'').startsWith('alloggio-')&&Number(o.quantity)>0)).map(p=>'<label class="mi-accommodation-person"><input type="checkbox" name="person" value="'+p.id+'"> '+esc(p.name)+' · '+esc(p.room||'da assegnare')+'</label>').join('')+'</fieldset><label>Nuova sistemazione<select name="type">'+Object.entries(types).map(([code,t])=>'<option value="'+esc(code)+'">'+esc(t.name)+'</option>').join('')+'</select></label><label>Numero della camera esistente (facoltativo)<input name="number" type="number" min="1" max="999999" step="1" placeholder="Nuova camera"></label><p>Per spostare le persone selezionate in una camera esistente, indica il suo numero. Lascia vuoto per creare una nuova camera: condivisa dalle persone selezionate per doppie e triple, con codici individuali per singole e camerate.</p><details data-accommodation-notes><summary>Annotazioni (facoltative)</summary><label>Annotazioni<textarea name="reason" maxlength="500"></textarea></label></details><div data-accommodation-preview aria-live="polite"></div><div class="mi-room-sticky-actions"><span data-accommodation-action-status>Nessuna persona selezionata</span><button type="button" data-discard-accommodation>Annulla</button><button type="submit" class="mi-primary" data-preview-accommodation>Verifica il cambio</button><button type="button" class="mi-primary" data-confirm-accommodation hidden>Conferma cambio e importi</button></div></form>';
          planner.append(changePanel);
          const assignmentPanel=document.createElement('section');assignmentPanel.dataset.roomModePanel='assign';assignmentPanel.className='mi-room-operation-panel';assignmentPanel.innerHTML='<h4>Assegna stanze</h4>';for(const node of [...planner.children])if(node!==changePanel)assignmentPanel.append(node);changePanel.dataset.roomModePanel='change';changePanel.classList.add('mi-room-operation-panel');changePanel.open=true;const modeControls=document.createElement('div');modeControls.className='mi-room-mode-controls';modeControls.innerHTML='<button type="button" data-room-mode="assign" aria-expanded="true">Assegna stanze</button><button type="button" data-room-mode="change" aria-expanded="false">Cambia tipo di abitazione</button>';planner.prepend(modeControls,assignmentPanel,changePanel);changePanel.hidden=true;const activateRoomMode=mode=>{assignmentPanel.hidden=mode!=='assign';changePanel.hidden=mode!=='change';for(const button of modeControls.querySelectorAll('button'))button.setAttribute('aria-expanded',button.dataset.roomMode===mode?'true':'false');};modeControls.onclick=e=>{const button=e.target.closest('[data-room-mode]');if(button)activateRoomMode(button.dataset.roomMode);};
        ...
        content.querySelector('[data-export]').onclick=async()=>{
          ...
          const rows=[selected.map(([key,label])=>reportLabel(key,label)),...exportRows.map(row=>selected.map(([key])=>key==='name'?row.name:key.startsWith('field:')?(row.fields?.[key.slice(6)]??''):key==='attendance'?({PRESENT:'Presente',ABSENT:'Assente',UNRECORDED:'Non rilevata'}[row.attendance?.state||row.attendance]||'Non rilevata'):key==='status'?stateLabel(row):key==='offer_expires_at'?deadlineLabel(row[key]):['paid','balance'].includes(key)?Number(row[key])/100:Array.isArray(row[key])?row[key].join(', '):row[key]))];
        ...
        const draw=async(append=false,provided=null)=>{
          ...
          if(individual){
            ...
            content.querySelectorAll('[data-list] tbody tr').forEach((row,index)=>{
              ...
              if(missing.length){const attention=document.createElement('small');attention.className='mi-row-attention';attention.textContent=missing.join(' · ');row.querySelector('.mi-participant-name').append(attention);}
          ...
          if(individual && data.attendance_availability?.available){
            ...
            const heading=document.createElement('th');heading.scope='col';heading.className='mi-attendance-cell';heading.textContent='Presente';headerRow.insertBefore(heading,statusAnchor);if(statusHeading)headerRow.append(statusHeading);
    ...
    async function syncSheet(){
      if(!await canLeave())return;if(!event){await allPeople();return;}
    ...
    const participantOptionGroups=[['alloggio','Alloggio'],['supplemento','Supplementi'],['trasferimento','Trasferimenti'],['pranzo','Pasti'],['altro','Altro']];
    function participantOptionCategory(option,definition={}){
      const code=String(option.code||definition.code||''),name=String(option.name||definition.name||''),category=String(definition.category||'');
      if(code.startsWith('alloggio-')||category==='alloggio')return 'alloggio';
      if(code==='colazione'||code.startsWith('assicurazione-')||/assicurazione/i.test(name)||category==='supplemento')return 'supplemento';
    ...
    function participantOptionsGroups(options,definitions=[]){
      const definitionByCode=new Map(definitions.map(definition=>[definition.code,definition]));
      const grouped=(Array.isArray(options)?options:[]).filter(option=>Number(option.quantity??1)>0).reduce((all,option)=>{const category=participantOptionCategory(option,definitionByCode.get(option.code));(all[category]??=[]).push(option);return all;},{});
      return participantOptionGroups.filter(([category])=>grouped[category]?.length).map(([category,label])=>'<section class="mi-person-option-group" data-person-option-group="'+category+'"><h4>'+label+'</h4><p>'+grouped[category].map(option=>esc(String(option.name||option.code||'Opzione').replace(/\s*[—–]\s*/g,' - '))).join(' · ')+'</p></section>').join('');
    }
    function optionChangeText(change){
      const before=new Map((change?.before_options||[]).map(option=>[option.code||option.name,option]));
      const after=new Map((change?.after_options||[]).map(option=>[option.code||option.name,option]));
      return [...new Set([...before.keys(),...after.keys()])].map(code=>{const oldOption=before.get(code)||{},newOption=after.get(code)||{},oldQuantity=Number(oldOption.quantity||0),newQuantity=Number(newOption.quantity||0),name=newOption.name||oldOption.name||code;if(oldQuantity===newQuantity)return '';if(!oldQuantity)return 'Aggiunto: '+esc(name)+(newQuantity>1?' ('+newQuantity+')':'');if(!newQuantity)return 'Rimosso: '+esc(name);return esc(name)+': '+oldQuantity+' → '+newQuantity;}).filter(Boolean).join(' · ');
    }
    ...
    function fieldInput(f,value){
      const name='field:'+f.key,label=f.label;
    ...
    async function detail(code,personNumber){
      ...
      try{const b=await request('detail');if(ticket!==generation)return;booking=b;printList=null;parkPanels();dirty=false;select.disabled=false;const selectedPerson=b.participants.find(p=>p.number===Number(personNumber))||b.participants[0];currentPerson=selectedPerson?.number??null;document.title=(selectedPerson?personName(selectedPerson):'Iscritto')+' — Scheda iscritto';
        ...
        const personalBalances=position?.people?.reduce((sum,p)=>sum+p.balance,0)||0,personalCredits=position?.people?.reduce((sum,p)=>sum+(p.credit||0),0)||0;
        const bookingOverview='<details class="mi-booking-overview" data-booking-overview><summary>Vedi tutta la prenotazione</summary><h4>'+esc(b.event_title)+'</h4>'+b.participants.map(p=>'<section><h4><button type="button" data-open="'+esc(b.order_code)+'" data-person-focus="'+p.number+'">'+esc(personName(p))+'</button>'+(p.status==='CANCELLED'?' · Annullata':'')+'</h4>'+participantOptionsGroups(p.options,b.option_definitions)+(b.features?.rooms?'<p>Camera assegnata: '+esc(p.room||'Da assegnare')+'</p>':'')+(!b.is_free_event?economics(position?.people?.find(row=>row.id===p.id)):'')+'</section>').join('')+((b.order_options||[]).length?'<h4>Servizi della prenotazione</h4>'+participantOptionsGroups(b.order_options,b.option_definitions):'')+(!b.is_free_event?'<section class="mi-booking-totals"><h4>Totali della prenotazione</h4><p>Quota '+money(b.total_cents)+' · Versato '+money(b.paid_cents)+' · '+(position?.quotes_known&&position?.payments_known?'Residui individuali '+money(personalBalances)+(personalCredits?' · Crediti da restituire '+money(personalCredits):''):'Residuo netto '+money(b.balance_cents))+'</p></section>':'')+'</details>';
        content.querySelector('[data-person-intro-end]').insertAdjacentHTML('beforebegin',bookingOverview);
        if(selectedPerson){
          ...
          const contacts=personal.map(f=>selectedPerson.fields?.[f.key]).filter(Boolean);contact.textContent='Contatti dell’iscritto: '+(contacts.join(' · ')||(selectedPerson.number===1?[b.buyer.email,b.buyer.phone].filter(Boolean).join(' · '):'Non indicati'));
          const companions=b.participants.filter(p=>p.id!==selectedPerson.id&&p.status==='ACTIVE').map(personName);if(companions.length)contact.insertAdjacentHTML('afterend','<p>Iscritto insieme con <strong>'+esc(new Intl.ListFormat('it',{style:'long',type:'conjunction'}).format(companions))+'</strong>.</p>');
        }
        if(b.can_change_options&&['CONFIRMED','PENDING_PAYMENT'].includes(b.status)&&(b.option_definitions||[]).length){
          const adjustmentForm=b.can_adjust_due?'<details><summary>Rettifica il dovuto della prenotazione</summary><form data-adjust-due><label>Persona<select name="participant_id" required>'+b.participants.map(p=>'<option value="'+p.id+'">'+esc(p.first_name+' '+p.last_name)+'</option>').join('')+'</select></label><label>Nuovo totale della prenotazione<input name="total" inputmode="decimal" required></label><label>Motivo<textarea name="reason" required maxlength="500"></textarea></label><button>Verifica e salva rettifica</button></form></details>':'';
          const optionForm=(id,title,scope,selected)=>{const definitions=b.option_definitions.filter(o=>o.scope===scope&&!String(o.code||'').startsWith('alloggio-')&&o.choice_group!=='alloggio'&&o.category!=='alloggio');const person=scope==='TICKET'?b.participants.find(p=>p.id===id):null;
          const types=person?b.option_definitions.filter(o=>o.scope==='TICKET'&&b.room_types?.[o.code]):[];
          const currentType=(person?.options||[]).find(o=>Number(o.quantity)>0&&b.room_types?.[o.code])?.code||'';
          const assignment=person? (types.length?'<label>Tipo di alloggio<select name="accommodation_type"><option value="">Nessun alloggio</option>'+types.map(o=>'<option value="'+esc(o.code)+'" '+(o.code===currentType?'selected':'')+'>'+esc(o.name)+'</option>').join('')+'</select></label><label>Camera<select name="room"><option value="">Da assegnare</option><option value="__AUTO__">Crea una nuova camera adatta</option>'+(b.accommodations||[]).map(r=>'<option value="'+esc(r.code)+'" '+(r.code===person.room?'selected':'')+'>'+esc(r.name||r.code)+' ('+Number(r.available)+' posti liberi)</option>').join('')+'</select></label>':'')+(b.bus_assignment_enabled?input('bus','Pullman assegnato',person.fields?.pullman||''):''):'';
          if(!definitions.length&&!assignment)return '';const grouped=definitions.reduce((all,definition)=>{const category=participantOptionCategory(definition,definition);(all[category]??=[]).push(definition);return all;},{});const fields=participantOptionGroups.filter(([category])=>grouped[category]?.length).map(([category,label])=>'<fieldset class="mi-person-option-group" data-person-option-group="'+category+'"><legend>'+label+'</legend>'+grouped[category].map(o=>'<label>'+esc(o.name)+(scope==='TICKET'?'<input type="checkbox" name="option:'+esc(o.code)+'" value="1" '+(Number((selected||[]).find(item=>item.code===o.code)?.quantity||0)>0?'checked':'')+'>':'<input type="number" min="0" max="'+Number(o.max_quantity??1)+'" step="1" name="option:'+esc(o.code)+'" value="'+Number((selected||[]).find(item=>item.code===o.code)?.quantity||0)+'">')+'</label>').join('')+'</fieldset>').join('');return '<form data-change-options="'+id+'"><h4>'+esc(title)+'</h4>'+fields+assignment+'<button type="submit">Verifica e salva servizi</button></form>';};
          content.insertAdjacentHTML('beforeend',adjustmentForm+optionForm(0,'Servizi comuni della prenotazione','ORDER',b.order_options));
          const serviceForms=selectedPerson&&selectedPerson.status==='ACTIVE'?optionForm(selectedPerson.id,'Servizi di '+personName(selectedPerson),'TICKET',selectedPerson.options):'';
          if(serviceForms)content.insertAdjacentHTML('beforeend','<details data-person-services-editor><summary>Varia servizi e sistemazione</summary><p>Modifica servizi, tipo di alloggio e assegnazioni insieme. Prima di salvare vedrai la nuova quota. Incassi e rimborsi si registrano separatamente.</p>'+serviceForms+'</details>');
        }
        const optionHistory=(b.option_changes||[]).filter(entry=>Number(entry.change?.participant_id)===Number(selectedPerson?.id)).map(entry=>({entry,changes:optionChangeText(entry.change)})).filter(item=>item.changes);
        if(optionHistory.length)content.insertAdjacentHTML('beforeend','<details><summary>Storico variazioni servizi</summary><ul>'+optionHistory.map(({entry,changes})=>'<li>'+changes+(entry.change.reason?' · '+esc(entry.change.reason):'')+' · '+esc(entry.actor_label)+' · '+esc(entry.created_at)+' UTC</li>').join('')+'</ul></details>');
        for(const p of selectedPerson?[selectedPerson]:[]){const definitions=new Map(b.fields.map(f=>[f.key,f]));Object.keys(p.fields).forEach(k=>{if(k==='pullman'&&!b.bus_assignment_enabled)return;if(!definitions.has(k))definitions.set(k,{key:k,label:k});});
          const selectedOptionGroups=participantOptionsGroups(p.options,b.option_definitions);
          const selectedOptions=selectedOptionGroups?`<div class="mi-person-options" aria-label="Servizi e sistemazioni scelti">${selectedOptionGroups}</div>`:'';
          const roomStatus=b.features?.rooms?'<p>Stanza: <strong>'+esc(p.room||'—')+'</strong></p>':'';
    ...
    content.addEventListener('submit',async e=>{if(e.target.closest('[data-mi-payments]'))return;e.preventDefault();const f=e.target;const invalid=[...f.elements].find(x=>x.willValidate&&!x.validity.valid);if(invalid){const closed=invalid.closest('details');if(closed)closed.open=true;invalid.setAttribute('aria-invalid','true');say('Controlla il formato del campo evidenziato.');invalid.focus();return;}const values=Object.fromEntries(new FormData(f));if(f.hasAttribute('data-change-options')){const options={};f.querySelectorAll('input[type=checkbox][name^="option:"]').forEach(el=>options[el.name.slice(7)]=el.checked?1:0);for(const [key,value]of Object.entries(values))if(key.startsWith('option:'))options[key.slice(7)]=Number(value);const change={participant_id:Number(f.dataset.changeOptions),options,reason:''};for(const key of ['accommodation_type','room','bus'])if(Object.hasOwn(values,key))change[key]=values[key];if(f.dataset.sharesReady){change.allocations={};for(const el of f.querySelectorAll('[data-share]'))change.allocations[el.dataset.share]=Math.round(Number(el.value.replace(',','.'))*100);}const button=f.querySelector('button[type=submit]');button.disabled=true;f.inert=true;busy=true;try{const preview=await request('options_preview',{data:JSON.stringify(change),version:booking.version});if(change.participant_id===0&&preview.allocations){
const fields=booking.participants.map(person=>'<label>'+esc(person.first_name+' '+person.last_name)+'<input data-share="'+person.id+'" inputmode="decimal" value="'+(Number(preview.allocations[person.id]||0)/100).toFixed(2)+'" required></label>').join('');
let editor=f.querySelector('[data-common-shares]');if(!editor){editor=document.createElement('fieldset');editor.dataset.commonShares='';f.querySelector('button').before(editor);}editor.innerHTML='<legend>Ripartizione dei servizi comuni</legend><p>Proposta in parti uguali. Modifica gli importi e conferma nuovamente.</p>'+fields;
if(!f.dataset.sharesReady){f.dataset.sharesReady='1';return;}
change.allocations={};for(const el of f.querySelectorAll('[data-share]'))change.allocations[el.dataset.share]=Math.round(Number(el.value.replace(',','.'))*100);
}
const assignmentText=preview.assignment&&Object.keys(preview.assignment).length?'Camera: '+(preview.assignment.room||'da assegnare')+'. Pullman: '+(preview.assignment.bus||'non assegnato')+'. ':'';
