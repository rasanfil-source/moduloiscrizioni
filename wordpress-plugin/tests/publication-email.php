...
function get_option($key,$default=null){return ['mi_modalita_spedizione_email'=>'PROVA','mi_destinatario_prova_email'=>'test@example.invalid','mi_email_segreteria_eventi'=>'secretariat@example.invalid'][$key]??$default;}
...
function wp_schedule_single_event(...$args){$GLOBALS['scheduled']=true;}
...
class EmailTestDB {
 ...
 function query($sql){$this->db->query($sql);return $this->db->affected_rows;}
}
...
$rows=$wpdb->db->query('SELECT * FROM wp_mi_email_outbox')->fetch_all(MYSQLI_ASSOC);expect(count($rows)===1&&$rows[0]['recipient']==='group@example.invalid'&&$rows[0]['status']==='TEST_PENDING','destinatario, test e deduplicazione');
$again=MI_Spedizione_Email::accoda_notifica_gestore_evento(42,null,'https://docs.google.com/spreadsheets/d/demo','group@example.invalid');expect($again['count']===0,'attivazione duplica pubblicazione');
