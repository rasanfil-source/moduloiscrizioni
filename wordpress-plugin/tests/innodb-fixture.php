...
class MI_Registration_Service {static function validate_options(...$args){return MI_Registration_Validation_Test::validate_options(...$args);}static function reopened_payment_deadline($registration,$now=null){$now=$now??time();$deadline=trim((string)($registration['payment_deadline_at']??''));$timestamp=$deadline===''?false:strtotime($deadline.' UTC');return false!==$timestamp&&$timestamp>$now?$deadline:gmdate('Y-m-d H:i:s',$now+48*3600);}static function append_registration_event(...$v){if(class_exists("MI_Booking_Update_Email")&&class_exists("MI_Registration_Validation_Test"))return MI_Registration_Validation_Test::append_registration_event(...$v);global $wpdb;if(empty($GLOBALS['management_audit_test']))return true;return false!==$wpdb->insert('wp_mi_registration_events',['registration_id'=>$v[0],'event_type'=>$v[1],'from_status'=>$v[2],'to_status'=>$v[3],'actor_label'=>$v[4],'detail_json'=>json_encode($v[5]??[]),'created_at'=>gmdate('Y-m-d H:i:s')]);}static function accoda_iscrizione_workspace($id){} static function mark_workspace_changed_locked($id){global $wpdb;if(1!==$wpdb->query($wpdb->prepare("UPDATE wp_mi_registrations SET workspace_revision=workspace_revision+1,workspace_status='PENDING',workspace_attempts=0 WHERE id=%d",$id)))throw new RuntimeException('Revision failed');}}
