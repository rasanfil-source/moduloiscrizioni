...
class MI_Registration_Service {static function append_registration_event(...$v){return true;}static function accoda_iscrizione_workspace($id){} static function mark_workspace_changed_locked($id){global $wpdb;if(1!==$wpdb->query($wpdb->prepare("UPDATE wp_mi_registrations SET workspace_revision=workspace_revision+1,workspace_status='PENDING',workspace_attempts=0 WHERE id=%d",$id)))throw new RuntimeException('Revision failed');}}
