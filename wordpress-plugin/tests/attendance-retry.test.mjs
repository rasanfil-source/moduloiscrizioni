...
const requestSource=source.slice(source.indexOf('    const attendanceRetries='),source.indexOf('    if(sheetButton)sheetButton.addEventListener'));
test('attendance network retries retain the request key; later edits invalidate stale keys',async()=>{
 ...
 await assert.rejects(c.request('attendance_bulk',present));
 await assert.rejects(c.request('attendance_bulk',present));
