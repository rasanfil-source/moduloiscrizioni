...
- Pagamenti presenta per prima la persona trovata dalla ricerca. Le spunte identificano i partecipanti a cui viene attribuito l’incasso; il totale è calcolato dal server e deve coincidere esattamente con caparra, saldo o residuo complessivo delle persone selezionate. Il saldo richiede caparra già coperta per ogni persona; Totale paga l’intero residuo. Nessuna tendina negli eventi a pagamento unico. Le attribuzioni sono salvate con il movimento nella stessa transazione e incluse nell’identità della richiesta per il retry. I versamenti precedenti senza attribuzione in prenotazioni multiple e le rettifiche o quote comuni non attribuite richiedono verifica: non sono ripartiti automaticamente. La scheda persona non mostra Presenza effettiva; le tessere dell’elenco mostrano la data d’iscrizione al posto del codice.

- Le tessere del riepilogo omettono «Confermata» e non ripetono «Pagamento atteso» quando mostrano già il residuo da versare. Lista d’attesa, posto proposto, annullamento e scadenza restano visibili. La ricerca Pagamenti esclude gli eventi dichiarati gratuiti (`pricing_mode=ZERO`) prima della paginazione; i titoli restituiti per ricerca e dettaglio sono testo con entità HTML decodificate. In Comunicazioni la modalità PROVA prepara l’invio al destinatario di test configurato per gli eventi pubblicati; negli altri casi la schermata conserva la preparazione senza invio e lo dichiara esplicitamente.
...
- Gestione camere offre Tutti gli stati / Assegnate / Da assegnare / Da riempire / Da riempire e da assegnare, inizialmente Tutti gli stati. Questa descrizione riflette i controlli già presenti nel codice prima della campagna del 22 settembre, senza cambiare il comportamento. Il filtro resta selezionato dopo il salvataggio; come il cambio di sistemazione, è bloccato con bozze o salvataggi pendenti. Le colonne sono Sistemazione, Numero, Persona; il numero si sceglie da un select nativo che considera disponibilità e progressivi. I riferimenti di iscrizione restano interni.
