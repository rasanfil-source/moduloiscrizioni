...
$destination = Join-Path (Resolve-Path 'dist').Path 'modulo-iscrizioni-3.26.131.zip'
...
[IO.File]::WriteAllText($destination + '.sha256', "$hash  modulo-iscrizioni-3.26.131.zip`n")
Write-Output $destination

$workspaceZip = Join-Path (Resolve-Path 'dist').Path 'Workspace-3.26.131.zip'
...
[IO.File]::WriteAllText($workspaceZip + '.sha256', "$workspaceHash  Workspace-3.26.131.zip`n")
