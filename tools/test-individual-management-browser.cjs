...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 assert.equal(saves,1);assert.equal(await page.locator('form[data-person]').getAttribute('data-person'),'2');assert.match(await page.locator('[aria-label="Importi personali"]').innerText(),/300,00/);
 await page.locator('[data-booking-overview] summary').click();assert.match(await page.locator('[data-booking-overview]').innerText(),/600,00/);await page.locator('[data-booking-overview] [data-person-focus="1"]').click();await page.locator('form[data-person="1"]').waitFor();assert.match(await page.locator('[aria-label="Importi personali"]').innerText(),/200,00/);
