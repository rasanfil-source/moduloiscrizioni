...
}else data={features:{rooms:true,payments:true,deposit:false},room_types:types,rooms,rooms_version:'v1',people,items};res.setHeader('Content-Type','application/json');return res.end(JSON.stringify({success:true,data}));}
...
const planner=page.locator('[data-room-planner]'),selector=planner.locator('select[data-room-type=""]');await page.locator('[data-room-section] > summary').click();await selector.waitFor();

const panel=page.locator('[data-accommodation-change]');await planner.locator('[data-room-mode="change"]').click();
...
await panel.locator('[data-accommodation-notes] summary').click();await panel.locator('[name=reason]').fill('Annotazione aggiunta');assert.ok(await panel.locator('[data-confirm-accommodation]').isHidden());
