...
$destination = Join-Path $dist 'modulo-iscrizioni-3.26.113.zip'
...
[IO.File]::WriteAllText($destination + '.sha256', "$hash  modulo-iscrizioni-3.26.113.zip`n")
Copy-Item -LiteralPath 'workspace-apps-script/src/EliminazioneEvento.gs' -Destination (Join-Path $dist 'EliminazioneEvento-3.26.113.gs')
