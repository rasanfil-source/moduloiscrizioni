...
La versione WordPress `0.5.2` e Apps Script `1.1.0` sono installate e collegate. La `0.5.3` è in preparazione e aggiunge nel dettaglio iscrizione lo stato operativo Workspace, i tentativi, l’ultimo errore, la data di sincronizzazione e un riaccodamento protetto per singola replica. La gratuità `ZERO`, la validazione delle combinazioni economiche e il collegamento firmato allo schema Workspace `1.1.0` sono già stati collaudati senza creare iscrizioni. Il deployment resta in modalità `ANTEPRIMA`; non vengono memorizzati IBAN o collegamenti carta, non esiste alcuna riscossione e la coda email resta `PREVIEW`.
