...
La versione WordPress `0.5.1` e Apps Script `1.1.0` sono installate e collegate. La `0.5.2` è in preparazione: rende esplicita la gratuità `ZERO`, applica escaping contestuale ai segnaposto email, normalizza una sola volta la configurazione economica e accoda la replica Workspace dopo il salvataggio locale. Il controllo firmato non invasivo dello schema Workspace è stato collaudato con esito positivo. Il deployment resta in modalità `ANTEPRIMA`; non vengono memorizzati IBAN o collegamenti carta, non esiste alcuna riscossione e la coda email resta `PREVIEW`.
