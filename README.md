...
La versione WordPress `0.5.3` e Apps Script `1.1.0` sono installate e collegate. Il dettaglio iscrizione mostra stato operativo Workspace, tentativi, ultimo errore e data di sincronizzazione; il riaccodamento protetto è disponibile soltanto per repliche non ancora sincronizzate. Pannello e collegamento firmato allo schema Workspace `1.1.0` sono stati collaudati senza creare iscrizioni. Il deployment resta in modalità `ANTEPRIMA`; non vengono memorizzati IBAN o collegamenti carta, non esiste alcuna riscossione e la coda email resta `PREVIEW`.
