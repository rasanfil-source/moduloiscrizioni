...
La versione WordPress `0.5.2` e Apps Script `1.1.0` sono installate e collegate. La gratuità `ZERO`, la validazione delle combinazioni economiche e il collegamento firmato allo schema Workspace `1.1.0` sono stati collaudati senza creare iscrizioni. I segnaposto email usano escaping contestuale e la replica Workspace viene accodata dopo il salvataggio locale. Il deployment resta in modalità `ANTEPRIMA`; non vengono memorizzati IBAN o collegamenti carta, non esiste alcuna riscossione e la coda email resta `PREVIEW`.
