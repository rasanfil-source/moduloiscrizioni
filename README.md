...
La versione WordPress `0.5.3` e Apps Script `1.1.0` sono installate e collegate. La `0.6.0` è in preparazione: comprende i controlli Workspace della `0.5.4` e rinnova il percorso pubblico con copertina evento, schede di iscrizione, tre passaggi e modalità concentrata priva della navigazione del tema. Il dettaglio iscrizione mostra stato operativo, tentativi, ultimo errore e data di sincronizzazione; il riaccodamento protetto è disponibile soltanto per repliche non ancora sincronizzate. Il deployment resta in modalità `ANTEPRIMA`; non vengono memorizzati IBAN o collegamenti carta, non esiste alcuna riscossione e la coda email resta `PREVIEW`.
