...
// Sorgente: Config.gs
const MI_SCHEMA_VERSION = '1.12.0';
...
function preparaProduzioniEventoConLock_(payload) {
  ...
  if (eventi.getMaxColumns() < 11) eventi.insertColumnsAfter(eventi.getMaxColumns(), 11 - eventi.getMaxColumns());
...
function inizializzaScheda_(sheet, headers) {
  ...
  const previous = MI_LEGACY_HEADERS[sheet.getName()] || [];
	...
	const immediatelyPrevious = sheet.getName() === MI_SHEETS.PARTICIPANTS ? [headers.slice(0, -5), headers.slice(0, -7)] : ([MI_SHEETS.REGISTRATIONS, MI_SHEETS.PAYMENTS, MI_SHEETS.EVENTS].indexOf(sheet.getName()) >= 0 ? [headers.slice(0, -1)] : []);
...
function preparaAperturaFoglio_(payload) {
  ...
  try {
    ...
    SpreadsheetApp.flush();
    return {ok:true,ready:complete,event_sheet_complete:complete,url_foglio:complete?result.url_foglio:undefined};
  } finally {lock.releaseLock();}
}
