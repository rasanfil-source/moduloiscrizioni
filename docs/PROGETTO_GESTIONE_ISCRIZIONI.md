...
La suite comprende 230 test Node, verifiche PHP e InnoDB e prove browser con dati sintetici. La paginazione server è implementata con righe complete a blocchi e selezione condivisa per ricerca, CSV e stampa; i conteggi continuano a leggere l’evento intero sul server. Stato, evidenze e limiti sono in Verifica avanzamento (documento storico rimosso). È predisposto il candidato locale 3.26.0 (documento storico rimosso). Restano il collaudo sull’installazione WordPress/Workspace e la distribuzione: nessuna pubblicazione o invio è stato eseguito. Il dettaglio dell’audit iniziale resta in Audit gestionale e UX (documento storico rimosso).
