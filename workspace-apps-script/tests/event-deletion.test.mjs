...
function setup(count=2){
 ...
 const sheet=n=>({name:n,getName:()=>n,deleteRows(row,count){assert.equal(locked,true);calls.push({name:n,row,count});tables[n].splice(row-2,count);}});
 ...
 return {c,tables,payload,calls,deny:v=>{denied=v;},trashed:()=>trashed,locked:()=>locked};
}
