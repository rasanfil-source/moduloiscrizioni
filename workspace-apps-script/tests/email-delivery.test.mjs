...
function setup({sender='info@parrocchiasanteugenio.it',quota=10,fail=false}={}) {
 ...
 const ctx={Session:{getEffectiveUser:()=>({getEmail:()=>sender})},PropertiesService:{getScriptProperties:()=>({getProperty:key=> key === 'MI_EMAIL_SENDER' ? 'info@parrocchiasanteugenio.it' : 'test@example.invalid'})},LockService:{getScriptLock:()=>({tryLock:()=>true,releaseLock:()=>{}})},ottieniFoglioDiLavoroAssociato_:()=>({getSheetByName:()=>sheet}),SpreadsheetApp:{flush:()=>{}},MailApp:{getRemainingDailyQuota:()=>quota,sendEmail:o=>{sent.push(o);if(fail)throw Error('uncertain');}},console:{error:()=>{}}};
...
test('invalid reply address is rejected before recording or sending',()=>{for(const reply_to of ['a@example.invalid\r\nBcc: b@example.invalid','a@example.invalid,b@example.invalid','invalid']){const s=setup();assert.equal(s.send({mode:'OPERATIVO',reply_to}).error,'INVALID_REPLY_TO');assert.equal(s.sent.length,0);assert.equal(s.rows.length,1);}});
