...
const source=(await Promise.all(['PagamentiEvento.gs','AccessoGestione.gs'].map(n=>readFile(new URL('../src/'+n,import.meta.url),'utf8')))).join('\n');
test('la proiezione sostituisce bozze locali, filtra evento e ordina incassi e rimborsi',()=>{
 ...
 ctx.eventoPrevedeMovimenti_=()=>true; sheet.showSheet=()=>{};
